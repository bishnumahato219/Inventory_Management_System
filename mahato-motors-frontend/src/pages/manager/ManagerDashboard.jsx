  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-10 bg-slate-50 min-h-screen font-sans overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex self-start md:self-center items-center gap-2 bg-white px-4 py-2 rounded-2xl shadow-sm border border-slate-100 text-[10px] md:text-xs font-bold text-slate-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        ...
        <div className="relative overflow-hidden bg-slate-900 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 text-white shadow-2xl">
          <div className="bg-orange-500 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg">
            <IndianRupee size={20} />
          </div>
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border border-slate-100 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <span className="bg-red-50 text-red-600 px-4 py-1 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest self-start sm:self-center">
            {stats.lowStock?.length || 0} Alerts
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          {stats.lowStock?.map((car) => (
            <div key={car._id} className="p-4 md:p-6 bg-slate-50 rounded-2xl md:rounded-3xl border border-transparent hover:border-red-100 transition-all">
              <p className="text-[10px] md:text-xs font-black text-slate-800 uppercase line-clamp-1">{car.modelName}</p>
          ...
          {(!stats.lowStock || stats.lowStock.length === 0) && (
            <div className="col-span-full py-8 text-center text-slate-400 font-bold text-[10px] md:text-xs uppercase italic">
